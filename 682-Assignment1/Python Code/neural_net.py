...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    Z1= X@W1+b1
    # h1into2= (np.maximum(h1,0))
    A1= np.maximum(Z1,0)
    Z2=  A1@W2 +b2

    # # print(Z2.shape)
    # smaxout= np.copy(Z2)
    # smaxout = smaxout-np.max(smaxout,axis=1)[:,None]
    # smaxout= np.exp(smaxout)

    # smaxsums= np.sum(smaxout,axis=1)
    # smaxout=smaxout/ smaxsums[:,None]

    scores=Z2
    self.params['score'] = Z2
    ...
    sum_y_values=(sum_y_values/N)+ reg* np.sum(np.square(W1))+reg* np.sum(np.square(W2))
    loss=sum_y_values
    ...
    dW2=  (A1.T@dZ2)/N +2*reg*W2

    dA1= np.copy(A1)
    dA1[dA1>0]=1

    dZ1=   np.multiply( W2@dZ2.T , dA1.T).T
    Db1= np.mean(dZ1,axis=0)
    dW1=  (X.T@dZ1)/N +2*reg*W1
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
